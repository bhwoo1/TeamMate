...
const NoticeEditPage = (props: {params: {id: number}}) => {
    ...
    const updateNoticeMutation = useMutation(updateNotice, {
        onSuccess: () => {
            router.push(`/notice/${props.params.id}`);
        },
    ...
    const handleUpdate = (e: FormEvent) => {
        ...
        if (session?.user?.email && editedNotice) {
            updateNoticeMutation.mutate({id: Number(props.params.id), title: editedNotice.title, content: editedNotice.content, user: String(session.user.email)});
        }
    };
                ...
                <form onSubmit={handleUpdate} className="border border-gray-300 w-[500px]">
                    <div className="border-b border-gray-300 flex flex-row justify-between">
                        ...
                        <div className="m-2 text-sm text-gray-500 flex flex-row justify-end">
                            <button type="button" className="mr-4" onClick={() => router.push(`/notice/${props.params.id}`)}>취소</button>
