...
const NoticeClient = (props: {id: number}) => {
    ...
    if (isError) return <p className="text-center">Error: {error.message}</p>;
      ...
      const handleDelete = () => {
        if (session?.user?.email) {
            deleteNoticeMutation.mutate({ id: props.id, user: String(session.user.email) });
        }
    };
                ...
                <div className="flex flex-row justify-between">
                    ...
                    {notice?.postedadmin === session?.user?.name &&
                        <div className="flex flex-row justify-between w-[100px] text-sm text-gray-500">
                            <button className="flex-1" onClick={() => router.push(`/notice/edit/${props.id}`)}>수정</button>
