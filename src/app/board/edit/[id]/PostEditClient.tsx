...
const PostEditClient = (props: {id: number}) => {
    ...
    const handleUpdate = (e: FormEvent) => {
        ...
        if (session?.user?.email && editedPost) {
            updatePostMutation.mutate({id: Number(props.id), title: editedPost.title, content: editedPost.content, user: String(session.user.name)});
        }
    };
                ...
                <form onSubmit={handleUpdate} className="border border-gray-300 w-[500px]">
                    <div className="border-b border-gray-300 flex flex-row justify-between">
                        ...
                        <div className="m-2 text-sm text-gray-500 flex flex-row justify-end">
                            <button type="button" className="mr-4" onClick={() => router.push(`/board/${props.id}`)}>취소</button>
