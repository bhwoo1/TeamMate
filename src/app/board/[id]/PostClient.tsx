...
const PostClient = (props:{id: number}) => {
    ...
    if (isError) return <p className="text-center">Error: {error.message}</p>;
      ...
      const handleDelete = () => {
        if (session?.user?.email) {
            deleteNoticeMutation.mutate({ id: Number(props.id), user: String(session.user.name) });
        }
    };
                ...
                <div className="flex flex-row justify-between">
                    ...
                    {post?.posteduser === session?.user?.name &&
                        <div className="flex flex-row justify-between w-[100px] text-sm text-gray-500">
                            <button className="flex-1" onClick={() => router.push(`/board/edit/${props.id}`)}>수정</button>
