...
const BoardTemplate = ({teamID, posts}: {teamID: number, posts: ReceivedPost[]}) => {
    ...
    const notices = posts.filter((post) => post.isNotice); // 공지사항만 추출
    const regularPosts = posts.filter((post) => !post.isNotice); // 일반 게시글만 추출
    ...
    const handleWriteBtnClick = () => {
        if (isAdmin === 'noMember') {
            ...
            router.push(`/team/${teamID}/community/write`)
        }
    }
    ...
    return (
        <div className="p-8 w-full w-3/4">
            ...
            <div className="flex justify-end items-center mb-4 pt-4">
                    <button onClick={handleWriteBtnClick} className="px-4 py-2 bg-blue-500 text-white rounded flex items-center">
