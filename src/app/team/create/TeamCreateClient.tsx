...
const TeamCreateClient = () => {
  ...
  const handleSubmit = async ({
    ...
    if (newTeam.teamLogo && newTeam.teamLogo instanceof File) {
      try {
        ...
        // 업로드된 파일의 URL을 상태에 반영
        newTeam.teamLogo = fileData.url;

        console.log("파일 업로드 성공, 등록 실행");

        // 팀 등록 요청 실행
        if (session?.user?.email) {
          submitTeamMutation.mutate({
            team: { ...newTeam, teamLogo: fileData.url },
    ...
    } else if (typeof newTeam.teamLogo === "string" && newTeam.teamLogo) {
      // 기존 URL이 있는 경우 바로 등록 요청 실행
      if (session?.user?.email) {
        submitTeamMutation.mutate({
          team: newTeam,
          user: String(session.user.email),
  ...
  return (
    <form onSubmit={(e) => handleSubmit({ e, newTeam })}>
      {/* 이름 입력 */}
      <div>
        <label
          htmlFor="name"
